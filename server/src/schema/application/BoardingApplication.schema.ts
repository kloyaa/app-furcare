...
import { baseApplicationFields, baseSchemaOptions } from '../base/BaseApplicationSchema';
import { IBoardingApplication } from '../../_core/interfaces/schema/schema.interface';
import { addPaymentMethods, addPaymentVirtuals, paymentFields } from '../../mixins/PaymentMixin';
...
const boardingApplicationSchema = new Schema<IBoardingApplication>({
    ...
    schedule: {
        ...
        time: {
            ...
            validate: {
                validator: (time: string) => /^(0?[1-9]|1[0-2]):[0-5][0-9]\s?(AM|PM)$/i.test(time),
...
// Virtual fields
boardingApplicationSchema.virtual('extensionDays').get(function (this: IBoardingApplication) {
    return this.schedule?.originalDays
        ? this.schedule.days - this.schedule.originalDays
        : 0;
});

boardingApplicationSchema.virtual('extensionPrice').get(function (this: IBoardingApplication) {
