...
import { ApplicationStatusEnum, ApplicationTypeEnum, groomingPreferencesEnum } from '../../enum/application.enum';
...
export const validateUpdateApplicationStatus = (body: any) => {
    const schema = Joi.object({
        ...
        applicationType: Joi.string()
            ...
            .required()
            .messages({
                'any.only': `Application type must be one of: ${Object.values(ApplicationTypeEnum)}`,
            }),
        status: Joi.string()
            .valid(...Object.values(ApplicationStatusEnum))
            .optional()
            .messages({
                'any.only': `Status must be one of: ${Object.values(ApplicationStatusEnum).join(', ')}`,
            }),
    });
