...
export const isPasswordAlreadyUsed = async (
  user: Types.ObjectId,
  password: string
) => {
  try {
    ...
    for (const hash of passwords) {
      const passwordMatched: any = await bcrypt.compare(
        password,
        hash.password
      );
      if (passwordMatched) {
...
export const findLastChangePassActivityByUser = async (
  user: Types.ObjectId
) => {
  try {
    const activity: any = await Password.findOne({ user }).sort({
      createdAt: 'desc',
    });
